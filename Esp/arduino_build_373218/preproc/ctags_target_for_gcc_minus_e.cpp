# 1 "F:\\Git\\AnhQuoc\\Release\\Esp\\Board_1\\Board_1.ino"
# 1 "F:\\Git\\AnhQuoc\\Release\\Esp\\Board_1\\Board_1.ino"
# 2 "F:\\Git\\AnhQuoc\\Release\\Esp\\Board_1\\Board_1.ino" 2
# 3 "F:\\Git\\AnhQuoc\\Release\\Esp\\Board_1\\Board_1.ino" 2
# 4 "F:\\Git\\AnhQuoc\\Release\\Esp\\Board_1\\Board_1.ino" 2
# 5 "F:\\Git\\AnhQuoc\\Release\\Esp\\Board_1\\Board_1.ino" 2
# 6 "F:\\Git\\AnhQuoc\\Release\\Esp\\Board_1\\Board_1.ino" 2
# 7 "F:\\Git\\AnhQuoc\\Release\\Esp\\Board_1\\Board_1.ino" 2
//DHT sensor
# 9 "F:\\Git\\AnhQuoc\\Release\\Esp\\Board_1\\Board_1.ino" 2

//Light
# 12 "F:\\Git\\AnhQuoc\\Release\\Esp\\Board_1\\Board_1.ino" 2
# 13 "F:\\Git\\AnhQuoc\\Release\\Esp\\Board_1\\Board_1.ino" 2
# 14 "F:\\Git\\AnhQuoc\\Release\\Esp\\Board_1\\Board_1.ino" 2
...
DHT dht(2 /* what pin we're connected to*/, 22 /* DHT 22  (AM2302)*/);
//Class zone ************************************
class Clock{
public:
  Clock(bool interNet){
...
public:
    ...
    void WriteSetting(bool isAuto,String str){
      if(isOpen){
            ...
            File file = SD.open("setting.txt",(O_READ | O_WRITE | O_CREAT));
            if(isAuto){
              file.print("1");
              file.println(str);
            }else{
...
public:
    ...
    struct Condition GetCondition(char* line){
        ...
        //Get temp and humi
        char * info=strtok(__null, "|"); //Save second |""| to info
        //Serial.print("Info: ");
        //Serial.println(info);
        info = strtok(info, "/"); //Save first ""/ from info to info  
        con.temp = atof(info);
        con.humi = atof(strtok(__null, "/"));//...
        con.lux= atoi(strtok(__null,"/"));
        char *rgb=strtok(__null,"/\n");
        con.R=atoi(strtok(rgb," "));
        con.G=atoi(strtok(__null," "));
        con.B=atoi(strtok(__null," \n"));
        //Get time range
        char* ftime = strtok(line, "-");
        char* toTime = strtok(__null, "-\n");


        con.fHour = atoi(strtok(ftime,":"));

        con.fMinute = atoi(strtok(__null, ":"));

        con.tHour = atoi(strtok(toTime, ":"));

        con.tMinute = atoi(strtok(__null, ":"));
        return con;
    }
};
